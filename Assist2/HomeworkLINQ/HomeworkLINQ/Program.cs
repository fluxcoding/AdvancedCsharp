    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            Console.WriteLine("how many Songs start with the letter 'a'");
            var songsA = Songs.Where(x => x.Name.StartsWith("a")).ToList();
            Console.WriteLine("--------------------------------------------------------------------");



            //2 - how many artists end with letter 'a' (case insensitive)
            Console.WriteLine("how many artists end with letter 'a'");

            var aritstsA = Artists.Where(x => x.FullName.EndsWith("a")).ToList();
             foreach (var item in aritstsA)
            {
            ...
            Console.WriteLine("whats the name of the song with longest duration");

            var longestD = Songs.OrderByDescending(x => x.Duration).First();
            Console.WriteLine(longestD.Name);

            Console.WriteLine("--------------------------------------------------------------------");


            //4 - whats the total Duration of all Songs
            Console.WriteLine("whats the total Duration of all Songs");

            var totalDuration = Songs.Select(x => x.Duration).Sum();
            ...
            Console.WriteLine("how many albums have Songs longer than 300 seconds");

            var songsMoreThan300 = Albums.Where(x => x.Songs.Any(c=>c.Duration>300)).Count();
            Console.WriteLine(songsMoreThan300);
           


            Console.WriteLine("--------------------------------------------------------------------");
            //6 - print the names of the artists(separated with "--"), that have more than one album of PopRock genre
            Console.WriteLine("print the names of the artists that have more than one album of PopRock genre");

            var artistPopRock = Artists
                 .Select(x => new { Name = x.FullName, Almbumz = x.Albums.Where(y => y.Genre == Genre.PopRock).Count() })
                 .Where(k => k.Almbumz > 1);
            foreach (var artist in artistPopRock)
            {
            ...
            Console.WriteLine("the name of the album that has highest Average duration of a song");

            var highestAverage = Albums.Select(x => new { name = x.Name, average = x.Songs.Average(n => n.Duration) }).OrderByDescending(c => c.average).First();
            Console.WriteLine(highestAverage.name);

            Console.WriteLine("--------------------------------------------------------------------");


            //8 - how many characters has the song that has the shortest Duration
            Console.WriteLine("how many characters has the song that has the shortest Duration");

            var shortestSongChars = Songs.OrderByDescending(x => x.Duration).Last();
            Console.WriteLine(shortestSongChars.Name.Length);
            ...
            Console.WriteLine("the name and the genre of the album that has most songs");

            var mostSongs = Albums.Select(x => new { name = x.Name, numOfSongs = x.Songs.Count(), genre = x.Genre }).OrderByDescending(c => c.numOfSongs).First();
            Console.WriteLine($"Name: {mostSongs.name}, Genre: {mostSongs.genre}");

            Console.WriteLine("--------------------------------------------------------------------");


            //10 - print the name of the artist that has most songs
            Console.WriteLine("the name of the artist that has most songs");

            var artistMostSongs = Artists.Select(x => new { Name = x.FullName, Songs = x.Albums.Sum(y => y.Songs.Count) }).OrderByDescending(a => a.Songs).First();
            Console.WriteLine(artistMostSongs.Name);
            ...
            Console.WriteLine("artist(SoloArtist/Band) that has most albums published before year 2000");
